...
const ContaReceberListMUI = () => {
  ...
  const carregarContas = async () => {
    try {
      const response = await fetch('http://localhost:8080/contas-receber');
      if (response.ok) {
  ...
  const handleView = async (id) => {
    try {
      const response = await fetch(`http://localhost:8080/contas-receber/${id}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleReceber = async (id) => {
    if (window.confirm('Tem certeza que deseja marcar esta conta como recebida?')) {
      try {
        const response = await fetch(`http://localhost:8080/contas-receber/${id}/pagar`, {
          method: 'PATCH',
          headers: {
        ...
        if (response.ok) {
          carregarContas();
          alert('Conta marcada como recebida com sucesso!');
        } else {
          const errorText = await response.text();
          console.error('Erro ao receber conta:', response.status, errorText);
          alert(`Erro ao marcar conta como recebida: ${errorText || response.status}`);
        }
      } catch (error) {
  ...
  if (loading) {
    return (
      <Box sx={{ 
        ...
        <Typography>Carregando contas a receber...</Typography>
      </Box>
    );
  }
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexGrow: 1, flexWrap: { xs: 'wrap', md: 'nowrap' } }}>
            ...
            <FormControl size="small" sx={{ minWidth: 150 }}>
              ...
              >
                ...
                <MenuItem value="PAGA">Recebida</MenuItem>
                  ...
                  >
                    ...
                    <TableCell align="center">
                      <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'center' }}>
                        <Tooltip title={conta.status?.toUpperCase() === 'PENDENTE' ? 'Receber' : 'Conta já recebida ou cancelada'}>
                          <span>
                            <IconButton 
                              size="small" 
                              color="success"
            ...
            <Grid container spacing={3} sx={{ mb: 5 }}>
              ...
              <Grid item sx={{ width: '18%' }}>
                <TextField
                  ...
                  value={contaSelecionada.dataPagamento ? formatarData(contaSelecionada.dataPagamento) : 'Não recebida'}
                  InputProps={{ readOnly: true }}
                  size="small"
                  variant="outlined"
                />
...
export default ContaReceberListMUI;
