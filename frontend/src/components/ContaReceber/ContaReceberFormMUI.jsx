...
const ContaReceberFormMUI = () => {
  ...
  const carregarDados = async () => {
    try {
      const [clientesRes, formasPagamentoRes] = await Promise.all([
  ...
  const carregarConta = async () => {
    try {
      ...
      setError('Erro ao carregar conta a receber');
    }
  };
  ...
  const handleClienteChange = (event, newValue) => {
    ...
    setConta(prev => ({ ...prev, clienteId: newValue?.id || '' }));
    if (errors.clienteId) {
  ...
  const getClienteLabel = (cliente) => {
    if (!cliente) return '';
    // PJ: razaoSocial (nomeFantasia)
    if (cliente.razaoSocial) {
      return `${cliente.razaoSocial}${cliente.nomeFantasia ? ` (${cliente.nomeFantasia})` : ''}`;
    }
    // PF: nome apelido
    return `${cliente.nome || ''}${cliente.apelido ? ` (${cliente.apelido})` : ''}`;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isEditMode 
        ? `http://localhost:8080/contas-receber/${id}`
        : 'http://localhost:8080/contas-receber';
      ...
      if (response.ok) {
        alert(`Conta a receber ${isEditMode ? 'atualizada' : 'cadastrada'} com sucesso!`);
      ...
      setError('Erro ao salvar conta a receber');
    } finally {
      setLoading(false);
    }
  };
...
export default ContaReceberFormMUI;
