...
// Componente de formulário de marca
const MarcaFormMUI = ({ id: propId, isModal = false, onClose }) => {
    ...
    if (id) {
      fetch(`http://localhost:8080/marcas/${id}`)
        ...
        .then((data) => {
          ...
          console.log('Marca final:', marcaAtualizada);
          setMarca(marcaAtualizada);
        })
        .catch((error) => console.error('Erro ao buscar marca:', error));
    }
  }, [id]);
  ...
  const handleChange = (e) => {
    ...
    setMarca({ ...marca, [name]: type === 'checkbox' ? checked : value });
  };
  ...
  const handleSubmit = (e) => {
    ...
    const marcaFormatada = {
      ...marca,
    };

    console.log('Dados enviados:', marcaFormatada);

    const method = id ? 'PUT' : 'POST';
    const url = id ? `http://localhost:8080/marcas/${id}` : 'http://localhost:8080/marcas';
    ...
    fetch(url, {
      ...
      body: JSON.stringify(marcaFormatada),
    })
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        if (isModal) {
          onClose(data); // Passa a marca criada/atualizada
        } else {
          navigate('/marcas');
        }
      })
      .catch((error) => {
        ...
        setErrorMessage('Erro ao salvar marca. Tente novamente.');
      });
  };
    ...
    <Box sx={{ 
      ...
      >
        ...
        <Box sx={{ 
          ...
          >
            {id ? 'Editar Marca' : 'Cadastrar Nova Marca'}
          </Typography>
          ...
          <Stack spacing={0.5} sx={{ flex: 1 }}>
            {marca.dataCriacao && (
              <Typography variant="caption" color="text.secondary">
                Data de cadastro: {new Date(marca.dataCriacao).toLocaleString('pt-BR')}
              </Typography>
            )}
            {marca.ultimaModificacao && (
              <Typography variant="caption" color="text.secondary">
                Última modificação: {new Date(marca.ultimaModificacao).toLocaleString('pt-BR')}
              </Typography>
            )}
          </Stack>
